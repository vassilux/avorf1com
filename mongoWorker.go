...
func (mongoWorker *MongoWorker) FetchOne() (result *Event, err error) {
	event := Event{}
	collection := mongoWorker.MongoDatabase.C("events")
	err = collection.Find(bson.M{"transport": "f1com"}).One(&event)
	if err != nil {
		return nil, err
	}
	return &event, nil
...
func (mongoWorker *MongoWorker) Fetch() (results []Event, err error) {
	collection := mongoWorker.MongoDatabase.C("events")
	err = collection.Find(bson.M{"transport": "f1com"}).All(&results)
	if err != nil {
		return nil, err
	}
	return results, nil
